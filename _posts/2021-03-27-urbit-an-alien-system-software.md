...
actors.  There's an hierarchy named after astronomy terms (galaxy, star,
planet), called points.  In addition  to them, there're Urbit identities
not  registered on  Azimunth, moons  and  comets.  Moons  are meant  for
devices  owned  by   a  planet  owner,  and   comment's  are  disposable
...
reputation  for  cryptoccurencies  doesn't   help  the  project  either.
Talking about reputation, it  means that philosophical disagreements may
lead  to  fracture  the  of  the  network,  something  already  seen  in
reputation-based online  communities. From same communities  it has also
been seen that people refrain from talking and behaving freely.
...
Also, there's a similarity in blockchain and Arvo event log being append
only.     Interestingly   Urbit    address   space    [is   a    special
case][urbit-not-bc] of a consensus ledger,  but Urbit [lacks a consensus
engine][urbit-bc].
...
in C.  The  design is similar.  Arvo is (compiled  to) Nock code running
atop a  Nock interpreter  implemented in  C.  Also,  Hoon and  Nock draw
elements from Lisp.  Albeit those languages  meant to be comparable to C
and assembly respectively.

Among the  differences is that they  don't share goals and  Emacs exists
only in application layer.  That is  its networking is like in any other
common application.
...
a virtual machine, and written entirely  in its own language. There's no
such thing as an identity layer and it uses the common network stack.

Received  from  its  ancestor  are  Venti and  Fossil.   Fossil  is  the
filesystem offering versioning feautures, and  Venti is a storage system
that enforces write-once  policy. Therefore they're similar  to Clay and
Arvo's event-log.
...
and even embedded systems) rather being  coupled to Unix.  Also Venti is
only used  for data rather  permanently store the imperative  steps that
...
GNUnet, started in 2001, is an interesting but unpopular project usually
overlooked in disqussions about  the decentralized ecosystem.  A thought
worth exploring is whether Urbit can  run on the GNUnet name system, and
whether it could be used to provide ownership (identity).
...
This takes a while.  When is  finished loading (or booting), its console
(called _dojo_) will  be shown.  The ship is a  persistent process.  You
can abandon the ship (pun intended,  abandon is not an actual term) with
^D or entering `|exit` into dojo.
...
```
dojo> |ota (sein:title our now our) %kids
```
...
You  can see  all commands  and available  tools, alongside  an one-line
summary running
...
important part which I didn't discussed in this post.

Closing  up,  Urbit is  one  of  the  _cool, cooler,  coolest_  projects
happening.   If   you  pass  over   the  arcane  jargon  used   and  the
cryptoccurency relation  (Ethereum; soft coupled), it  is an interesting
